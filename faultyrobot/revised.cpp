...
int main(void) {
	...
	if (hasRed) {
		...
		cout << (int)adjList[0].size() + 1 << endl;
	}
	*/
	findStopNodes(0, false);
	cout << endNodes.size() << endl;
...
/*	
	if (isBug && (int)adjList[node].size() == 0) {
...
*/
	...
	// If there is no red outgoing edges or there are no outgoing edges at all
	// This is an end point node
	//if ((redNodes.count(node) == 0) && ((isBug) || (int)adjList[node].size() == 0)) {
